...
def screenForUnintendedProducts(worker_num, fwd_rev_primers, transcriptome_fasta, expected_primed_isoforms, expected_primer_products, temp_dir,
                                high_stringency_loci, job_queue, results_queue):
    ...
    split_transcriptome_fasta = job_queue.get()
    while (split_transcriptome_fasta != "STOP"):
        ...
        print("INFO: worker", worker_num, "primer scanning", split_transcriptome_fasta, file=sys.stderr)

        log_file = split_transcriptome_fasta.replace(".fa", ".log")
        ...
        try:
            if (not os.path.isfile(usearch_output_file)): # TEMPORARY. REMOVE !!!!!!!!!!!!
                check_call(usearch_cmd, stdout=op_log, stderr=op_log)
        ...
        for line in ip:
            ...
            if (fields[0] == curr_isoform_ID):
                ...
                num_isoforms += 1
                calcPrimerAndProductPenalties(curr_isoform_ID, curr_isoform_matches, transcriptome_ref, primer_lookup, expected_primed_isoforms, expected_primer_products, 
                                              primer_hyb_scores, primer_hyb_penalties, product_penalties, product_scores, is_high_stringency_locus, op_log)
                curr_isoform_ID = fields[0]
                curr_locus = curr_isoform_ID.split('.',1)[0]
        ...
        results_queue.put( (worker_num, num_isoforms, usearch_output_file, primer_hyb_scores, primer_hyb_penalties, product_scores, product_penalties) )
        op_log.close()
        split_transcriptome_fasta = job_queue.get()
...
def calcPrimerAndProductPenalties(isoform_ID, isoform_matches, transcriptome_ref, primer_lookup, expected_primed_isoforms, 
                                  ...
                                  op_log, ignore_product_len=1000, rxn_temp_C=57, primer_concentration=1.0*10**-8):
    ...
    for isoform_strand, isoform_start, isoform_stop, primer_ID, primer_strand, num_mismatch in isoform_matches:
        ...
        if (template_start < 0 or template_stop > len(transcriptome_ref[isoform_ID])):
            continue
        else:
        ...
        if (frac_duplexed_any >= frac_duplex_cutoff):
            print("%s%s %s %s mm\n %s\n %s" % (section_header, full_pprID, primer_type, num_mismatch, primer_seq, template_seq), file=op_log)
            section_header = ''
            print(" Fraction duplexed: %6.5f" % frac_duplexed_any, file=op_log)
            ...
            if (this_product_len >= ignore_product_len or tup_3p[0] < tup_5p[0]):
                ...
                if (any(map(lambda cid: isoform_ID in expected_primed_isoforms[cid], common_full_pprIDs))): # If the current isoform is a target of the primer pair
                    ...
                    if ((tup_5p[0], tup_3p[1]) in product_start_stops_in_isoforms):
                        ...
                        print("WARNING: product start/stop positions didn't match up", file=op_log)
                else:
...
def formPool(G, max_clique_dyn_basename, temp_dir):
    primer_num_mishyb_cutoffs = [25, 50, 75, 100, 125, 25, 50, 75, 100, 125]
    sum_product_penalties_cutoffs = [0.001, 0.001, 0.001, 0.001, 0.001, 0.01, 0.01, 0.01, 0.01, 0.01]
    max_product_penalty_cutoffs = [1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3]

    solutions = []
    
    for index in xrange(len(primer_num_mishyb_cutoffs)):
        nodes_to_keep = []
        # Filter out nodes
        nodes_removed = []
        for n,D in G.nodes_iter(data=True):
            ...
            # For fwd_mishyb_ratio to be used, it needs to accumulate penalties only for where the corresponding rev primer has been retained.
            ...
            rev_mishyb_vals = map(itemgetter(1), D["rev_primer_hyb_penalties"])
            num_rev_mishyb = len(filter(lambda x: x>0.05, rev_mishyb_vals))
            #sum_rev_mishyb = sum(filter(lambda x: x>0.1, rev_mishyb_vals))
            #max_rev_mishyb = max(rev_mishyb_vals) if len(rev_mishyb_vals)>0 else 0

            #print(G.node[n]["full_pprID"], "num_rev_mishyb =", num_rev_mishyb, file=sys.stderr)
            #print >> sys.stderr, "%s rev num_mishyb = %d" % (G.node[n]["full_pprID"], len(filter(lambda x:x[1]>0.0001, D["rev_primer_hyb_penalties"])))
            rev_not_ok = D["rev_has_hairpin"] or D["rev_frac_homoduplex"]>0.01 or (D["rev_can_self_prime"] and D["rev_frac_homoduplex"]>0.001) or \
                         num_rev_mishyb > primer_num_mishyb_cutoffs[index]

            fwdrev_not_ok = D["fwdrev_frac_heteroduplex"]>0.01 or (D["fwdrev_can_heteroprime"] and D["fwdrev_frac_heteroduplex"]>0.001)

            if (not(any([fwd_not_ok, rev_not_ok, fwdrev_not_ok]))):
                nodes_to_keep.append(n)
            else:
                nodes_removed.append(n)
                #print("Removed node ", D["full_pprID"], file=sys.stderr)
                
        print("Removed", len(nodes_removed), "nodes", file=sys.stderr)
        ...
        for n1, n2, D in subG.edges_iter(data=True):
            ...
            sum_product_penalties = sum(product_penalties)
            exceeded_sum_product_penalties = sum_product_penalties > sum_product_penalties_cutoffs[index]
            ...
            if (fwd1rev2_not_ok or fwd2rev1_not_ok or exceeded_sum_product_penalties or exceeded_max_product_penalties):
                if (subG.node[n1]["full_pprID"].startswith("UDTD") or subG.node[n2]["full_pprID"].startswith("UDTD")):
                    #print(subG.node[n1]["full_pprID"], "and", subG.node[n2]["full_pprID"], file=sys.stderr)
                    #print(sum_product_penalties, max(product_penalties), len(product_penalties), file=sys.stderr)
                    #pdb.set_trace()
                    pass
                else:
        ...
        if (mcqd_output.startswith("Maximum clique: ")):
            ...
            print("INFO: Identified clique of size", len(clq), "\n", file=sys.stderr)
            if (len(clq) > 90):
...
if (__name__ == "__main__"):
    params_descriptor, database_path, scratch_dir_root, transcriptome_fasta, unprocessed_transcriptome_fasta, \
        ...
        adapter_primers_fasta, ribosomal_loci, output_place_picking, max_clique_dyn_basename = sys.argv[1:] # kmer_mm_graph_pkl
    ...
    if (data_for_graph):
        ...
        #except CalledProcessError as cpe:
        #    pdb.set_trace()
        split_transcriptome_fastas = glob.glob("%s/transcriptome_split_*.fa" % temp_dir)        

        job_queue = Queue(100)
        results_queue = Queue(30)
        worker_pids = []
        num_workers = 13

        for i in xrange(num_workers):
            p = Process(target=screenForUnintendedProducts, args=(i, fwd_rev_primers, transcriptome_fasta, expected_primed_isoforms,
                                                                  expected_primer_products, temp_dir, high_stringency_loci, job_queue, results_queue))
            p.start()
            worker_pids.append(p.pid)
        print("INFO: worker PIDs are ", ",".join(map(str, worker_pids)), file=sys.stderr)

        for split_transcriptome_fasta in split_transcriptome_fastas:
            job_queue.put(split_transcriptome_fasta)

        for j in xrange(num_workers):
            job_queue.put("STOP")
            
        all_primer_hyb_scores = defaultdict(list)
        all_primer_hyb_penalties = defaultdict(list)
        all_product_penalties = defaultdict(list)
        all_product_scores = defaultdict(list)

        for j in xrange(len(split_transcriptome_fastas)):
            worker_num, num_isoforms, usearch_output_file, primer_hyb_scores, primer_hyb_penalties, product_scores, product_penalties = results_queue.get()
            print("INFO: got results for", num_isoforms, "isoforms from worker", worker_num, ", usearch source file =", usearch_output_file, file=sys.stderr)

            for k, v in primer_hyb_scores.items():
                all_primer_hyb_scores[k].extend(v)

            for k, v in primer_hyb_penalties.items():
                all_primer_hyb_penalties[k].extend(v)
                
            for k, v in product_penalties.items():
                all_product_penalties[k].extend(v)
                
            for k, v in all_product_scores.items():
                all_product_scores[k].extend(v)
            
        #os.remove(usearch_output_file)
        #os.remove(split_transcriptome_fasta)

        #os.remove(primers_file)

        cPickle.dump((all_primer_hyb_scores, all_primer_hyb_penalties, all_product_scores, all_product_penalties), open("primer_and_product_penalties.pkl", 'wb'))
